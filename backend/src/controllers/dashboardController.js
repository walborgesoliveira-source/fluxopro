    ...
    try {
      ...
      const [
        ...
        pool.query(`SELECT COALESCE(SUM(valor),0) as total FROM movimentacoes
          ...
          [req.userId, m, a]),
        pool.query(
          `SELECT COALESCE(SUM(valor), 0) as total, COUNT(*)::integer as qtd
           FROM contas_pagar
           WHERE usuario_id = $1
             AND status = 'PAGO'
        ...
        pool.query(
          `SELECT
             c.id as cartao_id,
             BTRIM(c.nome) as cartao_nome,
             f.id as fatura_id,
             f.mes_referencia,
             f.ano_referencia,
             COUNT(cp.id)::integer as quantidade,
           ...
           WHERE cp.usuario_id = $1
             AND cp.status = 'PAGO'
             AND EXTRACT(MONTH FROM cp.data_pagamento) = $2
             AND EXTRACT(YEAR FROM cp.data_pagamento) = $3
           GROUP BY c.id, BTRIM(c.nome), f.id, f.mes_referencia, f.ano_referencia
           ORDER BY f.ano_referencia, f.mes_referencia, c.nome`,
          [req.userId, m, a]
        ),
        pool.query(
          `SELECT COALESCE(SUM(valor), 0) as total
           ...
           WHERE usuario_id = $1
             AND tipo = 'SAIDA'
             AND referencia_tipo = 'FATURA_CARTAO'
             AND EXTRACT(MONTH FROM data_movimentacao) = $2
             AND EXTRACT(YEAR FROM data_movimentacao) = $3`,
          [req.userId, m, a]
        ),
      ]);
      ...
      const compromissosCartao = pagamentosCartao.rows.map((r) => ({
        ...r,
        quantidade: parseInt(r.quantidade, 10),
        mes_referencia: parseInt(r.mes_referencia, 10),
        ano_referencia: parseInt(r.ano_referencia, 10),
      ...
      res.json({
        ...
        pagamentos: {
          conta_corrente: {
            total: parseFloat(pagamentosConta.rows[0].total),
            qtd: parseInt(pagamentosConta.rows[0].qtd, 10),
          },
          cartoes: compromissosCartao,
          total_cartoes: Number(
            compromissosCartao.reduce((total, item) => total + item.total, 0).toFixed(2)
          ),
          faturas_pagas: parseFloat(pagamentosFatura.rows[0].total),
        },
      });
